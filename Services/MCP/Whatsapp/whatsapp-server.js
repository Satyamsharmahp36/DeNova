...
import { MessageScheduler } from './scheduler-service.js';
...
const scheduler = new MessageScheduler(whatsappService, aiService);
...
/**
 * GET /api/whatsapp/contacts - Get WhatsApp contacts list
 */
app.get('/api/whatsapp/contacts', async (req, res) => {
    try {
        const { limit = 50 } = req.query;
        
        const result = await whatsappService.getContacts({ limit: parseInt(limit) });
...
/**
 * POST /api/whatsapp/contacts/send - Send message to contact by name
 */
app.post('/api/whatsapp/contacts/send', async (req, res) => {
    try {
        const { contactName, content } = req.body;
        
        if (!contactName || !content) {
            return res.status(400).json({
                success: false,
                message: 'Missing required fields: contactName, content',
                examples: {
                    basic: {
        ...
        const result = await whatsappService.sendMessageToContact(contactName, content);
        
        res.json({
            success: true,
            message: `Message sent to ${result.contactName} successfully`,
            data: {
                messageId: result.messageId,
        ...
        res.status(500).json({
            ...
            error: error.message,
            suggestion: 'Make sure the contact name matches a saved contact in your WhatsApp',
            timestamp: new Date().toISOString()
        });
    }
});
...
/**
 * POST /api/whatsapp/messages/schedule - Schedule a message
 */
app.post('/api/whatsapp/messages/schedule', async (req, res) => {
    try {
        const {
            sendMode,
            phoneNumber,
        ...
        if (!content || (!phoneNumber && !contactName)) {
            return res.status(400).json({
                success: false,
                message: 'Missing required fields: content and (phoneNumber or contactName)',
                examples: {
                    basic: {
                        sendMode: 'number',
                        phoneNumber: '+919882182880',
                        content: 'Hello! This is a scheduled message.',
                        scheduleTime: '2025-12-31T10:00:00.000Z',
                        aiEnhance: false
                    },
                    withAI: {
                        sendMode: 'contact',
                        ...
                        scheduleTime: '2025-12-31T15:30:00.000Z',
                        repeat: true,
                        repeatInterval: 'daily',
                        aiEnhance: true
                    }
...
/**
 * GET /api/whatsapp/messages/scheduled - Get all scheduled messages
 */
app.get('/api/whatsapp/messages/scheduled', async (req, res) => {
    try {
        const result = scheduler.getScheduledMessages();
        
        res.json({
            success: true,
            message: `Found ${result.total} scheduled messages`,
            data: result.jobs,
            total: result.total,
            timestamp: new Date().toISOString()
        });
...
/**
 * DELETE /api/whatsapp/messages/scheduled/:jobId - Cancel a scheduled message
 */
app.delete('/api/whatsapp/messages/scheduled/:jobId', async (req, res) => {
    try {
        const { jobId } = req.params;
        const result = scheduler.cancelScheduledMessage(jobId);
...
/**
 * GET /api/whatsapp/messages/history - Get job execution history
 */
app.get('/api/whatsapp/messages/history', async (req, res) => {
    try {
        const { limit = 50 } = req.query;
        const result = scheduler.getJobHistory(parseInt(limit));
        
        res.json({
            success: true,
            message: `Found ${result.total} history entries`,
            data: result.history,
            total: result.total,
            timestamp: new Date().toISOString()
        });
...
/**
 * POST /api/ai/whatsapp/enhance - Enhance message with AI
 */
app.post('/api/ai/whatsapp/enhance', async (req, res) => {
    try {
        const { content, platform = 'whatsapp', tone = 'friendly' } = req.body;
        ...
        const result = await aiService.enhanceWhatsAppMessage(content, {
            platform,
            tone
        });
