// ai-enhancement-service.js - Groq powered content generation for WhatsApp
...
export class WhatsAppAIService {
    constructor() {
        ...
        this.temperature = parseFloat(process.env.GENERATION_TEMPERATURE || '0.7');
        
        console.log('🤖 WhatsApp AI Enhancement Service Initialized with Groq');
...
WhatsApp Message:`;
            ...
            let generatedContent = (await this.generateContent(prompt)).trim();
...
Group Analysis:`;
            ...
            const analysis = (await this.generateContent(prompt)).trim();
...
Make general improvements for WhatsApp communication.`;
            ...
            let enhancedContent = (await this.generateContent(prompt)).trim();
    ...
    async testConnection() {
        try {
            const response = await this.generateContent("Say 'Hello from Groq!' and confirm you're working properly for WhatsApp content generation.");
            return {
                success: true,
                message: 'Groq AI service is working properly for WhatsApp',
                response: response,
                model: this.model
            };
        } catch (error) {
