...
const AccessControlPopup = ({ isOpen, onClose, taskId, userId, accessList, onSave }) => {
  ...
  const handleCreateBotAssistant = async (task) => {
    try {
      ...
      const botData = {
        name: `Bot Assistant for ${task.isMeeting.title}`,
        email: `bot_${task.uniqueTaskId}@chatmate.bot`,
        username: `bot_${task.uniqueTaskId}`,
        password: `bot_${Math.random().toString(36).substring(2, 15)}`,
        groqApiKey: import.meta.env.VITE_GROQ_API_KEY || "default_key",
        mobileNo: "0000000000",
        plan: "meeting",
        google: {
