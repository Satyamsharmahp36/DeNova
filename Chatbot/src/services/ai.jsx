...
async function detectTaskRequest(question, userData, conversationContext = "") {
  try {
    ...
    if (!groqApiKey) {
      return { isTask: false, error: "No Groq API key available" };
    }
...
export async function getAnswer(question, userData, presentData, conversationHistory = []) {
  try {
    const groqApiKey = import.meta.env.VITE_GROQ_API_KEY;
    console.log("Groq API Key loaded:", groqApiKey ? `${groqApiKey.substring(0, 10)}...` : "NOT FOUND");
    if (!groqApiKey) {
...
`;
    ...
    if (error.message.includes('API key')) {
      return "There was an issue with the API key. Please check your Groq API configuration.";
    }
